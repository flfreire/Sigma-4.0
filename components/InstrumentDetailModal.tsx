...
import { MeasurementInstrument, InstrumentStatus, CalibrationLog, ChecklistTemplate } from '../types';
...
const InstrumentDetailModal: React.FC<InstrumentDetailModalProps> = ({ isOpen, onClose, instrument, addInstrument, updateInstrument, checklistTemplates }) => {
    ...
    const metrologyTemplates = checklistTemplates.filter(t => t.type === 'metrology');
    ...
    const handleSave = async () => {
        if (isEditMode) {
            ...
            // FIX: The ID is now generated by the hook, so we remove it from the data before adding.
            const { id, ...dataToAdd } = formData;
            await addInstrument(dataToAdd);
        }
        onClose();
    };
    ...
    const renderGeneralTab = () => (
        <div className="space-y-4">
            ...
            <div>
                <label className="block text-sm font-medium text-highlight">{t('metrology.form.checklistTemplate')}</label>
                <select name="checklistTemplateId" value={formData.checklistTemplateId || ''} onChange={handleChange} className="mt-1 block w-full bg-primary border-accent rounded-md shadow-sm p-2">
                    <option value="">{t('metrology.form.noChecklist')}</option>
                    {metrologyTemplates.map(template => (
